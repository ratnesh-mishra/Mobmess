...
class MobileServer(HTTPService):
    ...
    def get_connection(self, _request: Request):
        ...
        if mobile_format_key in available_hosts:
    ...
    def receive_message(self, _request: Request):
        ...
        try:
            if tuple(peername) in self.available_hosts:
                message = payload.get('message', '')
                _to = payload.get()
                for users in self.available_hosts.keys():
                    if users.value == _to:
                        dest_add = users.key
                resp = yield from self.server_manager.send_message(message, dest_add)
                if resp:
